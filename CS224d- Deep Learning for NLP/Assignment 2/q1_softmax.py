...
def softmax(x):
  ...
  x = tf.subtract(x,tf.transpose([x_max]))
  x_exp = tf.exp(x)
  sum = tf.reduce_sum(x_exp,1)
  out = tf.divide(x_exp, tf.expand_dims(sum,axis = 1))

  '''''''''
  #code for debug
  with tf.Session():
      print("x",x.eval())
      print("x_max",x_max.eval())
      print(x_max.shape)
      print("x_exp", x_exp.eval())
      print("y", y.eval())
  '''''''''''
...
def cross_entropy_loss(y, yhat):
  ...
  ### YOUR CODE HERE
  y = tf.to_float(y)
  out = -tf.reduce_sum(tf.multiply(y, tf.log(yhat)), axis = 1)
