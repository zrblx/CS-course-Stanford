...
class SoftmaxModel(Model):
  ...
  def add_placeholders(self):
    ...
    """
    ### YOUR CODE HERE
    input_placeholder = tf.placeholder(tf.float32, shape = (Config.batch_size, Config.n_features))
    labels_placeholder = tf.placeholder(tf.float32, shape = (Config.batch_size, Config.n_classes))
  ...
  def add_model(self, input_data):
    ...
    """
    ### YOUR CODE HERE
    weight = tf.Variable(tf.zeros([Config.n_features, Config.n_classes]))
    bias = tf.Variable(tf.zeros([Config.batch_size, Config.n_classes]))
    input = tf.matmul(input_data, weight) + bias
