...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       % grad_theta{scale} = zeros(size(currentImage));
        grad_mag{scale} = sqrt(power(img_dx,2)+power(img_dy,2));
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
       ...
       % patch_angle_offset = 0;
        patch_angle_offset = ComputeDominantDirection(patch_mag, patch_theta,patch_size);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        feature = zeros(0);
        cells_mag = mat2cell(patch_mag,[4,4,4,4],[4,4,4,4]);
        cells_theta = mat2cell(patch_theta,[4,4,4,4],[4,4,4,4]);
        ...
        for n = 1:4
            for m = 1:4
                [histogram, ~] = ComputeGradientHistogram(num_angles, cells_mag{n,m}, cells_theta{n,m});
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [m,n] = size(gradient_angles);
    for y = 1:m
        for x = 1:n
            for k =1:num_bins
                if (k==num_bins) && (gradient_angles(x,y)>= angles(num_bins))
                    histogram(k)= histogram(k)+gradient_magnitudes(x,y);
                elseif (gradient_angles(x,y)<angles(k+1)) && (gradient_angles(x,y)>=angles(k))
                    histogram(k)= histogram(k)+gradient_magnitudes(x,y);
                    break;
                end
            end
...
function direction = ComputeDominantDirection(gradient_magnitudes, gradient_angles,scale)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    [histogram_do, angles_do] = ComputeGradientHistogram(num_bins, gradient_magnitudes, gradient_angles);
    % Step 2:
    ...
    maximum = max(histogram_do);
    [~,col] = find(histogram_do==maximum);
    direction = angles_do(col);
